#' Calculate Annual R99p (Extremely Wet Days)
#'
#' Calculates the total annual precipitation from days exceeding the 
#' 99th percentile of a 30-year baseline period.
#'
...
#' @param baseline_range Numeric Vector. Start and end year for baseline (e.g., c(1981, 2010)).
...
#' @param plot_result Logical. If TRUE, plots the annual results.
...
#'
#' @return A SpatRaster object containing the R99p values for each year.
#' @export
#' @importFrom terra rast vect crop mask time nlyr app time<- subset quantile global project crs writeRaster
#' @importFrom grDevices hcl.colors
#' @importFrom graphics par plot text
calculate_r99p <- function(nc_input, start_date = NULL, 
  ...
  # Fallback if baseline years are missing
  if (length(baseline_idx) == 0) {
    warning("Baseline years not found. Using all available data as baseline.")
  ...
  message("Calculating Annual R99p...")
  ...
  for (yr in unique_years) {
    ...
    extreme_rain <- yr_stack * (yr_stack > percentile_map)
    annual_sum <- sum(extreme_rain, na.rm = TRUE)
    
    annual_layers[[length(annual_layers) + 1]] <- annual_sum
    valid_years <- c(valid_years, yr)
  }
  
  result_stack <- terra::rast(annual_layers)
  names(result_stack) <- paste0("R99p_", valid_years)
  
  # --- 5. Plotting (Wrapped in TryCatch) ---
  if (plot_result) {
    tryCatch({
      ...
      # If map is completely flat (e.g. all 0), DO NOT use a legend
      if (isTRUE(min_val == max_val)) {
        ...
        terra::plot(result_stack[[1]], 
                    main = paste("Annual R99p - Year", valid_years[1]),
                    col = "lightgrey", 
                    legend = FALSE) 
      } else {
        # Normal Plotting
        extreme_pal <- hcl.colors(50, "Reds 3", rev = TRUE)
        terra::plot(result_stack[[1]], 
                    main = paste("Annual R99p (Extremely Wet Days) - Year", valid_years[1]), 
                    col = extreme_pal,
